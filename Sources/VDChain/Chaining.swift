...
extension ChainProperty {
	...
	public subscript<T, A>(dynamicMember keyPath: KeyPath<T, A>) -> ChainProperty<Base, A?> where Value == T? {
		ChainProperty<Base, A?>(chaining, getter: getter.appending(path: \.okp[keyPath]))
	}
	
	public subscript<T, A>(dynamicMember keyPath: WritableKeyPath<T, A?>) -> ChainProperty<Base, A?> where Value == T? {
...
extension Optional {
	fileprivate var okp: OKP<Wrapped> {
		get { OKP(optional: self) }
		set { self = newValue.optional }
	}
}
